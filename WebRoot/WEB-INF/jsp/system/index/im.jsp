...
layui.use(['layim','readyMenu'], function(layim){
  ...
  layim.on('sendMessage', function(data){
	 ...
	 *	mine.content：消息内容
	 */
	 websocket.send(",fhadmin-msg,"+user+",cms,"+uname+",cms,"+To.id+",cms,"+To.type+",cms,"+mine.avatar+",cms,"+mine.content);
  });
...
function removeIFromFriend(FUSERNAME){
	websocket.send(',fhadmin-remove,'+user+',cms,'+FUSERNAME);
}
...
function agreeFriend(FUSERNAME){
	websocket.send(',fhadmin-agreefriend,'+user+",cms,"+FUSERNAME);
}

//同意对方申请加群，发送同意信息给对方
function agreeQgroup(uid,group){
	websocket.send(',fhadmin-agreeqgroup,'+uid+",cms,"+group);
}

//踢出群，发踢出信息给对方
function kickoutQgroup(uid,group){
	websocket.send(',fhadmin-kickoutqgroup,'+uid+",cms,"+group);
}
